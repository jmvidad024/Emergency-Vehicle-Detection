...
def detect_vehicle():
    ...
    try:
        ...
        # --- THE FIX: Clean up any weird newline or space encodings from the web payload ---
        raw_base64 = raw_base64.replace(" ", "+").replace("\n", "").replace("\r", "")

        url = "https://detect.roboflow.com/coco/3"
        params = {"api_key": ROBOFLOW_API_KEY}
        
        # Post the raw string as form-data directly
        ...
        # 🧪 Log the raw response parameters to Render logs to diagnose what Roboflow sees!
        print(f"👁️ AI Detections: {detected_labels}")

        # Core Traffic Rules
        # COCO dataset recognizes 'truck', 'bus', 'car', 'motorcycle'
